...
- 輔助物：只保留 2–4 個與文章直接相關的 icon，先判斷文章是否真的需要人物。
...
1. **方向與理由**：選用的風格、底色、材質，以及它如何對應文章讀者與內容。
2. **閱讀動線**：文字區、hero 主視覺、icon 區與留白區的左右／上下分工；主標必須有明確的第一視覺焦點。
3. **主視覺層級**：一個 hero 物件或人物，搭配 2–4 個次要符號，不用增加元素數量取代構圖。
4. **icon 與裝飾字清單**：逐一指定 icon 的語意、位置、大小層級與是否需要獨立 badge；所有可讀文字列入白名單。
5. **不可變與禁止項目**：若是迭代既有好版本，列出要保留的構圖、主色、人物／物件與文字；禁止隨機 UI、小字、假 logo、網址、年份與未鎖定工具名。

使用內建 `image_gen` 工具；遵循 `imagegen` skill 的保存與檢查規則，最後一定要把專案使用的檔案放進 `themes/hexschool/source/images/`。

#### 吸睛構圖與 AI icon 裝飾

- 以「1 個 hero＋2–4 個 icon＋最多 3 層文字」建立視覺階層；保留可呼吸的留白，避免每個角落都放卡片、線條或文字。
...
- 若文章確實提到或使用 AI 工具，可加入 2–4 個相關工具 icon 作為次要裝飾，並將 icon 放在不干擾人物與文案的位置；icon 圖稿本身不要帶可讀標籤，但可依上方規則加入獨立、已鎖定的短 badge。優先使用可驗證的本機 icon 資產；若沒有可靠的品牌 icon，改用中性的 AI 工具符號（聊天泡泡、文件、圖片、影片、程式碼括號、workflow 節點或 automation spark），不要生成變形 logo、假品牌標誌或與文章無關的工具。若主題沒有必要的人物、產品或工具 icon，不要為了填版面硬塞。
...
- 2–4 個與文章主題確實相關的 AI 工具 icon 或中性 AI 視覺符號，例如聊天泡泡、文件、圖片、影片、workflow 節點、程式碼括號、資料流或 automation spark；先指定群組排列、材質與主次，再決定是否加入 3–5 個獨立 badge。icon 與 badge 只作裝飾及概念提示，不可冒充文章未提及的工具或合作品牌。
...
對 imagegen 產出的結果做三層視覺驗收：先以 `view_image` 檢查原尺寸的構圖與人物／主視覺，再以縮小預覽檢查左側文字層級、對比、色彩差異與列表縮圖可讀性，最後才做尺寸／格式驗證。先做一次「文字白名單」掃描：逐字核對 kicker、主標、hook、badge 與所有可讀字樣；中央 AI 核心、icon、卡片與背景 UI 不得出現未鎖定文字。縮圖至少要同時通過五項：主標第一眼可辨識、hero 仍是第二視覺焦點、icon 群組不超過 2–4 個且有主次、底色與近期封面有明顯差異、留白沒有被裝飾吃滿。若有錯字、變形、額外文字、裁切、文字過小、對比不足、主視覺消失、風格又退回泛用深色科技風或只剩泛泛口號，針對文案、色系與版面重新生成；不要把額外文字或元素數量當成吸睛而放行。若兩次以上仍不穩定，先向使用者回報，不要把明顯錯字或不可辨識的封面提交上線。
