---
name: add-hexo-post
description: 將使用者提供的文章初稿依專案內 prompt/hexo-post.md 整理成 SEO 友善的 Hexo 文章，先提供英文 slug 候選供選擇，再以 hexo new 的原始建立時間建立文章、清除 scaffold 殘留、補充 FAQ；生成或重做封面前先分析文章標題、內文、讀者轉變與近期封面分布，選定視覺方向、主標層級與人物適配，再以藝術指導與風格輪替生成或重做 1200x800 AI 封面，完成本機與正式站驗收，並在分支目的地明確確認後以繁體中文 commit 並 push 到 main。當使用者要新增、整理、發布、重做未提交文章封面或上線一篇 AI-Consultant Hexo 部落格文章時使用。
---
...
- 字級：kicker、主標、hook 的層級與行數；主標通常用 1–2 行，在安全區內以可用的最大尺寸呈現，但「超大」是驗收結果，不是固定比例或必選效果。
- 字效：粗體、描邊、陰影、色塊、漸層或發光；選擇縮小後仍能讀出的組合。
- 輔助物：只保留 0–4 個與文章直接相關的 icon，先判斷文章是否真的需要人物；icon 不是封面必備配額。

對 `assets/02-YT封面範例01.png`，優先萃取「上方小 kicker＋中央超粗兩行主標＋底部短 hook 色帶＋右側主視覺／icon 區」；只借鑑版型與字效，不把 Astro、Next.js、程式碼、分數、年份、人物、圖示、logo 或原始文案當成文章內容。超大主標、粗描邊／偏移陰影、強烈兩色與底部箭頭 hook 都是可選的設計語法，不是每張封面的固定四件套。參考圖只作 layout reference，不直接裁切、複製或正規化成 cover；若傳給 imagegen，使用絕對路徑並在 prompt 明確寫出不可複製的元素。
...
封面不是文章標題的裝飾背景；先從標題與內文找出「讀者要理解的核心轉變」，再決定主視覺、媒材、版型與人物。生成前寫一份可審核的短設計摘要，不要只寫「吸睛、好看、科技感」：

```text
文章核心承諾：標題要讀者知道什麼？
讀者痛點／轉變：從什麼狀態到什麼狀態？
內文可視化證據：3–5 個文章確實提到的名詞、操作或關係
視覺命題：用一句視覺隱喻表達上述轉變
風格方向：媒材、色彩、構圖語法，以及相對近期封面的輪替理由
主標編排：kicker、主標第 1 行、第 2 行、hook 的逐字文案
人物決策：加入／不加入；若加入，人物如何服務信任或內容理解
icon／badge：0–4 個必要元素與逐字白名單
禁止項目：泛用 AI 物件、假 logo、未鎖定小字、網址、年份與不實承諾
```

至少要把兩個「文章專屬語彙」轉成畫面關係，而不是只放工具名稱或泛用 AI 圖示。例如 n8n 自架 AI Assistant 應表現 trigger／Webhook → 分支節點 → AI／程式碼／資料 → Sandbox 的工作流關係；只放 `n8n` badge、腦袋、霓虹線或 3D 物件，不能算具備 n8n 感。其他工具也要找出它在文章中的實際機制，例如 API 的 request／response、資料整理的索引／分類、課程的學習路徑，而不是把名詞逐一畫成素材清單。

近期封面踩坑要蒸餾成以下放行規則：

| 失敗訊號 | 蒸餾規則 | 放行條件 |
| --- | --- | --- |
| 米色＋3D 看起來與文章無關 | 不把系列慣用風格當成預設答案 | 風格方向能由文章內容與讀者情境說明，且至少改變近期封面的兩個設計軸 |
| 有工具名稱但沒有產品感 | 品牌辨識來自操作語意，不是 badge 或顏色 | 主視覺拿掉標題後仍能表達文章機制，並有至少兩個主題專屬視覺關係 |
| 主標太小或被背景搶走 | 主標是第一視覺層，但改善順序是精煉文案、清理背景、調整層級，最後才放大 | 原圖與縮圖都先讀到主標，且主標逐字正確、對比足夠、在安全區內以 1–2 行呈現 |
| 超大主標壓滿畫面 | 「超大」不等於把完整標題塞進圖片；先保留核心承諾，再以卡片裁切反推尺寸 | 主標沒有貼邊、沒有跨出安全區，桌面／手機卡片都能讀完，且不需要縮到接近 kicker 的大小 |
| 粗描邊與陰影變成髒邊 | 描邊與陰影只負責分離前景，不是額外堆疊的裝飾 | 最多一層清楚描邊與一個同方向偏移陰影；中文字內部沒有被填死，且沒有再疊發光、3D bevel 或多重重影 |
| 兩色都在搶主標 | 顏色要表達語意層級，不是把兩行做成兩個平行標題 | 一個主色承擔主要訊息，強調色只標示一個關鍵詞或一行；即使移除顏色，仍能靠尺寸／位置讀出主次 |
| 底部箭頭 hook 像貼紙或 CTA | hook 只有在能表達流程、轉變或結果時才存在，箭頭是方向語意，不是填空裝飾 | 一句短文案、一個箭頭、位於底部安全區內且次於主標；不貼邊、不做按鈕、不重複主標，無功能時直接省略 |
| 為了填空間硬加人物 | 人物是內容／信任決策，不是裝飾配額 | 有明確加入理由，人物位置、比例與服裝不妨礙主標與主視覺 |
| 原圖完整但文章列表裁切主標 | 原圖驗收不代表實際元件驗收 | 桌面與手機列表卡片的實際截圖中，主標、產品標記、臉部與關鍵符號均未被 `object-fit: cover` 裁掉 |
...
依文章內容與近期封面分布，從下列方向選一個，並在 imagegen prompt 中明確寫出方向、配色、材質與選擇理由。方向不是依文章分類自動套用；文章分析卡中的內容證據優先於模板慣性：
...
人物只有在能增加文章理解或品牌信任時才出現。先做決策，再選位置與肖像，不要先生成一個人物再想辦法塞進版面：

| 文章情境 | 預設決策 | 版面要求 |
| --- | --- | --- |
| 作者觀點、教學、顧問服務、實作示範、個人品牌 | 加入 | 人物可作右側／三分之一主角，保留文案安全區，臉部與眼鏡清楚可辨 |
| 工具教學或操作指南 | 視內容決定 | 只有當人物能示範「誰在使用、如何落地」才加入；若流程圖更能說明機制，省略人物 |
| API／架構／資料流／工具比較／抽象觀念 | 通常省略 | 以流程、資料關係、介面或物件建立命題，避免人物搶過技術語意 |

若人物決策為「加入」，必須同時記錄：人物來源、不可變的臉部／眼鏡／髮型／服裝特徵、位置與比例、與背景光線的相容性，以及人物不遮擋主標的理由。若沒有可驗證的專案內肖像、參考圖無法讀取、生成後身份特徵失真，改採不含人物版型，不用虛構人物補空間。人物驗收要看原尺寸與縮圖，確認人物沒有被裁切成只剩臉、被 icon 遮住，或因背景風格不合而看起來像另一篇文章。

#### 主標層級、文案鎖定與安全區

主標編排要先於 imagegen。先鎖定精確文案、斷行與文字盒尺寸，再讓 imagegen 產生無文字或低文字背景，最後以 SVG／HTML／影像合成等可控方式放入繁體中文與工具名稱；不要把關鍵中文交給生成模型自由拼字。最多三層主文案，再加一個必要 badge：

1. `kicker`：文章類型、工具或系列標籤，最小，只作入口。
2. `主標`：從標題與內文抽出最短核心承諾，粗體、大字、1–2 行，整張圖第一視覺焦點；尺寸由安全區、縮圖可讀性與卡片裁切共同決定，不以「越大越吸睛」作為單一目標。
3. `hook`：一句痛點／轉變或操作結果，補充主標，不重複完整文章標題。

主標可用「白色主句＋品牌色關鍵詞」、「兩行不同色塊」、「描邊／偏移陰影」或「大字與短 hook 的尺度對比」建立層次；這些效果要服務文章語意與閱讀順序，不要同時把描邊、陰影、發光、漸層與 3D 立體邊全疊上去。不要把完整長標題縮成小字。所有可讀文字逐字列入白名單，並禁止背景 UI、卡片、icon 或 AI 核心自行補入文字。

以 1200×800 原圖為基準，主標、kicker、重要 badge 與人物臉部／品牌符號的左右至少預留 72px、上下至少預留 56px，預先假設文章列表卡片會以 `object-fit: cover` 裁掉約 5–8% 邊緣。原圖完成後，必須額外輸出桌面列表卡片與手機列表卡片的實際截圖；若任何關鍵文字被裁掉，優先調整合成版面的安全區，不要只宣稱原圖完整。

#### 主標排版復盤：借鑑設計語法，不複製副作用

參考封面的價值在於「先建立視覺閱讀順序」，不是把某一張圖的效果全部搬過來。主標設計可借鑑大字、粗描邊／偏移陰影、兩色層級與底部箭頭 hook，但每一項都要先回答它服務哪個訊息；若只增加聲量、厚度或裝飾，應退回重排。

| 元素 | 可保留的意圖 | 常見副作用 | 執行規則 |
| --- | --- | --- | --- |
| 超大主標 | 讓讀者在列表縮圖先讀到核心承諾 | 完整標題塞不下、貼邊、被 `object-fit: cover` 切掉，或背景只剩很小一塊 | 先縮短成核心主標，再在 1200×800 安全區內測量；允許大，但不能讓字跨出左右 72px／上下 56px，也不能壓縮到人物或產品語意 |
| 粗描邊／偏移陰影 | 在複雜背景上建立字與底圖的分離 | 筆畫變厚、中文字內孔被填死、重影太多，整張圖變得廉價或混濁 | 優先選「單層描邊」或「單一方向偏移陰影」；兩者同時使用時要降低其中一者，禁止再疊發光、多重陰影與 3D bevel；縮圖仍需看清字內留白 |
| 強烈兩色層級 | 讓關鍵詞或第二行有記憶點 | 兩行同樣大、同樣飽和，視覺變成兩個平行主標；顏色與文章語意無關 | 設一個主色、一個語意強調色；強調色最多承擔一個關鍵詞或一行，主次仍由尺寸、位置、留白與字重共同建立，不靠顏色單獨撐住 |
| 底部箭頭 hook | 用一句話把痛點連到轉變、流程或結果 | 變成貼紙、按鈕、口號或額外第三個主標；貼近底邊後在列表卡片消失 | 只在文章確實有「從 A 到 B」或流程方向時使用；保持一句短文案與一個箭頭，放在底部安全區且明顯小於主標，若只是填空或重複標題就刪除 |

排版執行順序固定為：**分析文章 → 鎖定主標與 hook 逐字文案 → 決定斷行與文字盒 → 選擇字效與色彩角色 → 選風格與背景 → deterministic overlay → 原圖／480px 縮圖／桌面卡片／手機卡片驗收**。若文字盒放不進安全區，先改寫或減少字數；不要以縮小主標、增加描邊或把 hook 貼到邊緣來掩蓋版面問題。

以下任一情況都要退回主標排版，不得只用「再加特效」修補：主標需跨越安全區才能放下、描邊吃掉中文字內部、兩色沒有語意主次、箭頭 hook 沒有轉變功能、縮圖先看到 hook／icon／人物而不是主標，或原圖正確但任一實際列表卡片裁掉關鍵字。若連續兩次調整仍無法同時保留主標可讀性與文章視覺命題，先回報取捨，不要把不穩定的效果提交。
...
- skill 內的 `.agents/skills/add-hexo-post/assets/01-人物基準圖－ 4K.jpg` 是「享哥 AI 應用講師」的可選人物品牌基準圖；若專案內另有已確認的作者肖像，也必須先用 `view_image` 檢查並記錄實際來源。生成含人物的封面前先選定一個可驗證來源，再以 `referenced_image_paths` 傳給 imagegen；所有人物來源都不可讀時不要自行捏造或用其他人物替代，可改採不含人物的版型並回報未使用人物基準圖。
...
- 主標必須是整張封面的第一視覺焦點：從文章主標與內文抽出最短的核心訊息，採粗體、大字、1–2 行呈現；字級與視覺重量要明顯大於 kicker、hook、工具 icon 與其他輔助文字，但尺寸必須受安全區、縮圖與卡片裁切約束。可依色系使用高對比色、單層描邊、單一偏移陰影或色塊；不要為了塞入完整標題而縮小主標，也不要把「超大＋粗描邊＋重陰影」當成固定配方；長標題應拆成短主標與 hook。
...
- 若使用底部箭頭 hook，只在文章確實有流程／轉變／結果方向時使用；它必須是一句短文案加一個箭頭，位於底部安全區且明顯次於主標，不得做成裝飾貼紙或 CTA 按鈕。
...
- 主標使用粗體、大字與清楚的視覺層次，確保縮小成文章列表縮圖後仍是第一個被讀到的元素；可參考 `assets/02-YT封面範例01.png` 的高對比描邊、陰影與分層效果，但需依文章選定的 `light-editorial`／`bright-3d`／`paper-collage`／`hybrid`／`dark-tech` 方向重新設計，並依上方復盤規則控制字效。
...
縮小預覽時，主標必須先於人物、hook、icon、badge 與背景特效被辨識；若主標不夠粗、不夠大、對比不足、和近期封面太相似或被其他元素搶走視線，先縮短文案、清理背景、調整文字盒與色彩／字效層級，再考慮放大；不要直接用更粗描邊、更重陰影或更大的字覆蓋版面問題。

文章列表的實際卡片是獨立驗收面：至少在桌面寬度與手機寬度各截圖一次，檢查 `cover` 元素的裁切框、主標安全區、人物臉部與產品識別元素。若只有原圖通過而列表卡片失敗，封面仍不得放行；先移動 deterministic overlay、縮小非必要裝飾或調整構圖，再重跑原圖與縮圖檢查。
